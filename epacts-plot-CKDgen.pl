...
wGetOptions(
    "-Generate Manhattan, QQ plot, and top signals from EPACTS results",
    "--Required Options",
    "in=s", [\$in,
    ...
    "regionf=s",[\$regionf,
	         "File containing [CHR] [START] [END] [COLOR]",
	         "File containing [CHR] [START] [END] [COLOR]"
		],
...
my $mcnt = 0;
my %hLabelf = ();
my (@regionChr, @regionStart, @regionEnd, @regionColor);
...
if ( $regionf ) {
    ...
    @regionEnd = ();
    @regionColor = ();
    open(REG,$regionf) || die "Cannot open $regionf\n";
    while(<REG>) {
        my ($chr, $begin, $end, $color) = split;
	push(@regionChr, $chr);
        push(@regionStart, $begin);
        push(@regionEnd, $end);
        push(@regionColor, $color);
    }
    close REG;
}
...
sub findRegionColor {
    my ($chr, $pos) = ($_[0], $_[1]);
    for (my $i = 0; $i <= $#regionStart; $i++) {
        if ($regionChr[$i] == $chr &&
	    $regionStart[$i] <= $pos &&
	    $regionEnd[$i] >= $pos) {
		return $regionColor[$i];
	}
    }
}
...
while(<IN>) {
	...
	push(@xs,$xbin);
	push(@ys,$ybin);
	my $regionColor = findRegionColor($chr, $pos);
	if ($regionColor) {
		push(@cs, $regionColor);
	} else {
...
print CMD "set terminal postscript eps enhanced dashed dashlength 3.0 linewidth 1.0 size 3,3.5 font 'Calibri,16' fontfile 'calibri.pfb' fontfile 'GillSansMT.pfb' fontfile 'GillSansItalic.pfb'\n";
...
open(CMD,">$outf.cmd") || die "Cannot open file\n";
print CMD "set terminal postscript eps enhanced dashed dashlength 3.0 linewidth 1.0 size 8,3 font 'Calibri,18' fontfile 'calibri.pfb' fontfile 'GillSansMT.pfb' fontfile 'GillSansItalic.pfb'\n";
